...
// CODE HERE
const btnColor = (event) => {
  event.target.style.backgroundColor = 'black'
  event.target.style.color = 'white'
}
sayHelloButton.addEventListener('mouseover', btnColor)
// PROBLEM 3
/*
...
// CODE HERE
const btnColorChangeBack = (event) => {
  event.target.style.backgroundColor = '#EFEFEF'
  event.target.style.color = 'black'
}
sayHelloButton.addEventListener(`mouseout`, btnColorChangeBack)
// PROBLEM 4
/*
...
const repeatMyParam = () => {
  axios
    .get(`http://localhost:3000/repeat/moneytree`)
    .then((response) => {
      console.log(response.data)
...
const getRequest = (event) => {
  axios
    .get('http://localhost:3000/query-test/?name=blobman&job=sleep')
    .then((response) => {
      console.log(response.data)
    })
    .catch((error) => {
...
/*
    ...
    Got an error in the terminal - [ERR_HTTP_HEADERS_SENT]: Cannot set headers after they are sent to the client.

    2: Send more than 1 query on the URL -- what happened? 
    you get a message saying you send more than one query
*/
